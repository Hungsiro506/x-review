...
and so users can extend it without editing the package:
  ...
  config:  repo-local .debate-review.yaml  >  user config  >  bundled default
  skills:  user skills dir  >  bundled skills

User dir defaults to ~/.config/debate-review (override with DEBATE_REVIEW_HOME).
"""
...
USER_DIR = Path(
    os.environ.get(
        "DEBATE_REVIEW_HOME",
        Path(os.environ.get("XDG_CONFIG_HOME", Path.home() / ".config")) / "debate-review",
    )
)
...
def load_repo_overrides(repo):
    """Per-repo .debate-review.yaml at the repo root, if present."""
    p = Path(repo) / ".debate-review.yaml"
    if p.exists():
        with open(p) as f:
...
def pick_skill_names(cli_skills, repo_overrides, config, language):
    """Order: --skills flag > .debate-review.yaml > language default."""
    if cli_skills:
        return cli_skills
...
def scratch_dir(repo_name, target_ref):
    ...
    return base / "debate-review" / repo_name / safe
