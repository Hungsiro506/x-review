#!/usr/bin/env bash
# Install quorum: the `quorum` command + a user-level Claude Code
...
SKILL_DIR="${HOME}/.claude/skills/quorum"
...
# 1b) pipx, if available.
  ...
  mkdir -p "${BIN_DIR}"
  cat > "${BIN_DIR}/quorum" <<EOF
#!/usr/bin/env bash
exec python3 "${TOOL_DIR}/review.py" "\$@"
EOF
  chmod +x "${BIN_DIR}/quorum"
  installed="shim"
fi
...
mkdir -p "${SKILL_DIR}"
cp "${TOOL_DIR}/quorum/data/SKILL.md" "${SKILL_DIR}/SKILL.md"
...
if ! command -v quorum >/dev/null 2>&1; then
  ...
  echo ""
  echo "⚠ 'quorum' not on PATH yet. Add to your shell rc:"
  echo "    export PATH=\"${HINT}:\${PATH}\""
fi

echo ""
echo "Done. Try:  cd <any-repo> && quorum --list-skills"
